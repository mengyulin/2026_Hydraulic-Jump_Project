...
Authored homepage stays at repository root. The sealed lab manifest determines
the ZIP contents; website additions never alter the original teaching release.
"""
...
def render_markdown(source):
    ...
    # Protect display equations from Markdown's underscore/emphasis parsing.
    equations = []
    def protect(match):
        equations.append(match.group(1))
        return f'\n\nEQUATIONPLACEHOLDER{len(equations)-1}END\n\n'
    text = re.sub(r'\$\$(.*?)\$\$', protect, text, flags=re.S)
    rendered = markdown.markdown(text, extensions=['tables', 'fenced_code', 'toc'])
    for i, equation in enumerate(equations):
        rendered = rendered.replace(f'<p>EQUATIONPLACEHOLDER{i}END</p>',
            f'<div class="math-display">\\[{html.escape(equation)}\\]</div>')
    def rewrite(match):
...
def package_lab():
    ...
    with zipfile.ZipFile(archive, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as output:
        for name in sorted([*manifest, 'CHECKSUMS.json']):
            info = zipfile.ZipInfo('gc1991-lab/' + name, (2026, 9, 14, 0, 0, 0))
